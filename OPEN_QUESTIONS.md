...
`src/data/vehicle-types/vehicleTypeViewConfig.tsx` defines `vehicleTypeFilters` with stop-place-era categories (`parentVehicleType`, `railVehicle`, `metroVehicle`, `onstreetBus`, `onstreetTram`, `ferryStop`, `harbourPort`, `liftStation`). The filter UI renders these chips, but the underlying `getFilterKey` does not produce values matching them — meaning none of these filters actually filter anything. Same stub bug also affects `src/data/deck-plans/deckPlanViewConfig.ts`.
