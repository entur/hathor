...
`test.describe.configure({ mode: 'serial' })` (e.g. `import-state-refresh.spec.ts`). The **live**
run (`e2e:local-backend`) pins `--workers=1` because parallel writes against one real Sobek would race.
